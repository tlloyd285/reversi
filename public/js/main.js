...
/* This function returns the value asociated with whichParam on the URL */
...
//what to do when someone leaves room
socket.on('player_disconnected', function (payload) {
    ...
    //if someone leaves room, then animate out their content
    var dom_elements = $('.socket_' + payload.socket_id);
    //if someone exists room, then animate out their content
    if (dom_elements.length != 0) {
    ...
    for(row = 0; row < 8; row++){
        for(column = 0; column < 8; column++){
            ...
            if(old_board[row][column] != board[row][column]){
                ...
                else{$('#'+row+'_'+column).removeClass('hovered_over');}
        }
    }
}
